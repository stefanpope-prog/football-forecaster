...
**Architecture:** Python pipeline (ingest → ratings → model → picks → form → publish) writes parquet to `data/` and HTML/markdown to `docs/`. Dixon-Coles bivariate Poisson with Elo-derived strengths produces an 8×8 scoreline grid per fixture; an EV optimizer over the adidas Forecaster scoring rule (3 outcome + 1 per-team-score + 1 goal diff = 6 max) picks the recommended scoreline. The dashboard shows quantitative context (recent form, Elo edge); deeper qualitative analysis (injuries, weather) is generated **on-demand by free Claude Chat** when the user taps "Open in Claude Chat" — no paid API key required. GitHub Pages serves the HTML; GitHub Actions runs the pipeline hourly on match days. Out of scope for v1: tournament/Final-Four Monte Carlo (separate v1.5 plan).

**Tech Stack:** Python 3.12, `httpx`, `pandas` + `pyarrow`, `numpy`, `scipy`, `jinja2`, `pytest`, GitHub Actions, GitHub Pages. **No paid LLM API.**
...
- Tasks 11, 12, 13 (HTML dashboard, prompt pack, recent-form summary) can run in parallel after Task 10.
...
- **Required secrets** (added in Task 15):
  - `FOOTBALL_DATA_API_KEY` — free key (Stefan has one). Goes into **GitHub Actions repo secrets**, never into source.
- **Stefan's GitHub username:** `stefanpope-prog` (personal account). Public repo URL will be `https://github.com/stefanpope-prog/football-forecaster` and Pages URL `https://stefanpope-prog.github.io/football-forecaster/`.
- **No paid LLM API.** Rationale is generated by free Claude Chat on-demand from the briefing the dashboard puts in the clipboard.
...
3. **Trust the probability numbers above as ground truth — don't recompute them.** Use qualitative inputs only as caveats ("model doesn't price in Mbappé being out").
"""
...
## Task 13: Form summary (deterministic, no LLM)

Generates the "Why" paragraph from data we already have: recent form (last 5 results per team from `historical.parquet` + finished WC matches), Elo gap, head-to-head, and venue. No external API call. Two reasons this is the right call:

1. **No paid LLM dependency.** Free, deterministic, runs in milliseconds.
2. **Deeper qualitative analysis (injuries, weather, lineups) lives in the "Open in Claude Chat" flow** — the briefing the dashboard copies to clipboard already gives free Claude Chat everything it needs to write injury-aware analysis on demand. That's strictly better than a stale paid LLM rationale baked at pipeline time.

**Files:**
- Create: `forecaster/form.py`
...
def _history():
    ...
    # MEX: W W D L W (last 5 in chrono order)
...
def test_build_form_summary_mentions_form_and_elo():
    ...
    # Should reference Elo edge or strength.
    assert "Elo" in text or "stronger" in text or "edge" in text or "favoured" in text
    # Should include some form indicator (W/D/L letters).
...
def test_summarise_all_returns_dict_keyed_by_fixture_id():
    ...
    assert len(out["2026-06-11-MEX-RSA"]) > 20   # non-trivial paragraph
```
...
Expected: 3 failures (`forecaster.form` doesn't exist).

- [ ] **Step 3: Implement `forecaster/form.py`**
...
```bash
git add forecaster/form.py tests/test_form.py
git commit -m "feat(form): deterministic per-fixture form summary (no LLM)"
```
...
- [ ] Create the **public** repo `football-forecaster` on `https://github.com/stefanpope-prog`.
- [ ] Push the local repo: `git remote add origin git@github.com:stefanpope-prog/football-forecaster.git && git push -u origin main`
- [ ] In repo **Settings → Pages**: Source = "GitHub Actions".
- [ ] In repo **Settings → Secrets and variables → Actions**: add a new repo secret `FOOTBALL_DATA_API_KEY` (value = the football-data.org key, regenerated). **Never commit this value.**
...
- [ ] `python -m forecaster run --force` produces `docs/index.html`, `docs/prompt-pack.md`, `docs/api/*.json` locally.
- [ ] `docs/index.html` opens correctly in Chrome on iOS (test on Stefan's iPhone) — try "Add to Home Screen".
- [ ] **[Open in Claude Chat]** button on a fixture card copies the briefing and opens claude.ai. Paste it; verify free Claude Chat can answer "should I pick 2-1 or 1-1?" using the supplied numbers.
...
- [ ] GitHub Pages serves the dashboard at https://stefanpope-prog.github.io/football-forecaster/
- [ ] No `ANTHROPIC_API_KEY` or other paid-API secret needed anywhere in the repo or CI.
